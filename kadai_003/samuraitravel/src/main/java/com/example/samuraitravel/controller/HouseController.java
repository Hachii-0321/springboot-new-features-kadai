...
import com.example.samuraitravel.repository.*;
...
public class HouseController {
    ...
    public String show(@PathVariable(name = "id") Integer id, Model model, @AuthenticationPrincipal UserDetailsImpl userDetailsImpl) {
        ...
        boolean favoriteAlreadyExists = false;
        if (userDetailsImpl != null) {
            User user = userRepository.getReferenceById(userDetailsImpl.getUser().getId());
            reviewAlreadyExists = reviewRepository.existsByHouseAndUser(house, user);
            favoriteAlreadyExists = favoriteRepository.existsByHouseAndUser(house, user);
        }

        model.addAttribute("house", house);
        model.addAttribute("newReviews", newReviews);
        model.addAttribute("reservationInputForm", new ReservationInputForm());
        model.addAttribute("reviewAlreadyExists", reviewAlreadyExists);
        model.addAttribute("favoriteAlreadyExists", favoriteAlreadyExists);
        return "houses/show";
    }
}